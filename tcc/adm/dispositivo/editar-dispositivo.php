...
$locais = $localDAO->listarTodos(); // Para dropdown

// Verifica se veio o ID do dispositivo
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: dispositivos.php?msg=dispositivo_nao_informado');
    exit;
}
...
if (!$dispositivo) {
    header('Location: dispositivos.php?msg=dispositivo_nao_encontrado');
    exit;
}
?>
    ...
    <main class="flex-grow flex items-center justify-center px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full bg-white dark:bg-background-dark rounded-xl shadow-lg p-8 space-y-6 border border-gray-200 dark:border-gray-700">
        <div class="text-center">
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Dispositivo</h2>
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Atualize as informações do dispositivo</p>
        </div>

        <!-- Formulário de edição -->
        <form method="post" action="../../process/dispositivo/process_editar_dispositivo.php" class="space-y-6">
          ...
          <div>
            ...
            <input type="text" name="nome" required value="<?= htmlspecialchars($dispositivo['nome']) ?>"
          ...
          <div>
            ...
            <input type="text" name="codigo_esp" required value="<?= htmlspecialchars($dispositivo['codigo_esp']) ?>"
              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-primary focus:border-primary focus:outline-none">
          </div>

          <div>
            <label for="ativo" class="inline-flex items-center mt-2">
              <input type="checkbox" name="ativo" <?= $dispositivo['ativo'] ? 'checked' : '' ?>
          ...
          <div>
            ...
            <select name="tbl_locais_id" required
              ...
              <?php foreach ($locais as $local): ?>
                <option value="<?= $local['id'] ?>" <?= $dispositivo['tbl_locais_id'] == $local['id'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($local['locais']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
